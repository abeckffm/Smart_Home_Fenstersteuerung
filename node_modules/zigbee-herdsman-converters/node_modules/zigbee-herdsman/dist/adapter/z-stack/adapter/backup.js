...
const debug = debug_1.default('zigbee-herdsman:adapter:zStack:backup');
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const [key, entry] of Object.entries(items)) {
            if (entry.product === PRODUCT_ALL || entry.product === product) {
                ...
                data[key] = Object.assign(Object.assign({}, entry), { value: [...result.payload.value], len: result.payload.value.length });
            }
        }
        return {
            adapterType: 'zStack',
...
function Restore(znp, backupPath, options) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if ((product !== tstype_1.ZnpVersion.zStack30x && product !== tstype_1.ZnpVersion.zStack3x0)
            || (backup.meta.product !== tstype_1.ZnpVersion.zStack30x && backup.meta.product !== tstype_1.ZnpVersion.zStack3x0)) {
            throw new Error('Backup is only supported for Z-Stack 3');
        }
        // 3.0                                                    3.x
        ...
        if (backup.meta.product == tstype_1.ZnpVersion.zStack30x && product == tstype_1.ZnpVersion.zStack3x0) {
            debug(`Migrate '${tstype_1.ZnpVersion[backup.meta.product]}' -> '${tstype_1.ZnpVersion[product]}'`);
            backup.data.ZCD_NV_EX_TCLK_TABLE = Object.assign(Object.assign({}, items.ZCD_NV_EX_TCLK_TABLE), { value: backup.data.ZCD_NV_LEGACY_TCLK_TABLE_START.value, len: backup.data.ZCD_NV_LEGACY_TCLK_TABLE_START.len });
            backup.data.ZCD_NV_EX_NWK_SEC_MATERIAL_TABLE = Object.assign(Object.assign({}, items.ZCD_NV_EX_NWK_SEC_MATERIAL_TABLE), { value: backup.data.ZCD_NV_LEGACY_NWK_SEC_MATERIAL_TABLE_START.value, len: backup.data.ZCD_NV_LEGACY_NWK_SEC_MATERIAL_TABLE_START.len });
        }
        else if (backup.meta.product == tstype_1.ZnpVersion.zStack3x0 && product == tstype_1.ZnpVersion.zStack30x) {
            debug(`Migrate '${tstype_1.ZnpVersion[backup.meta.product]}' -> '${tstype_1.ZnpVersion[product]}'`);
            backup.data.ZCD_NV_LEGACY_TCLK_TABLE_START = Object.assign(Object.assign({}, items.ZCD_NV_LEGACY_TCLK_TABLE_START), { value: backup.data.ZCD_NV_EX_TCLK_TABLE.value, len: backup.data.ZCD_NV_EX_TCLK_TABLE.len });
            backup.data.ZCD_NV_LEGACY_NWK_SEC_MATERIAL_TABLE_START = Object.assign(Object.assign({}, items.ZCD_NV_LEGACY_NWK_SEC_MATERIAL_TABLE_START), { value: backup.data.ZCD_NV_EX_NWK_SEC_MATERIAL_TABLE.value, len: backup.data.ZCD_NV_EX_NWK_SEC_MATERIAL_TABLE.len });
        }
        if (!es6_1.default(backup.data.ZCD_NV_CHANLIST.value, Constants.Utils.getChannelMask(options.channelList))) {
            throw new Error(`Cannot restore backup, channel of backup is different`);
        }
        if (!es6_1.default(backup.data.ZCD_NV_PRECFGKEY.value, options.networkKey)) {
            throw new Error(`Cannot restore backup, networkKey of backup is different`);
        }
        if (!es6_1.default(backup.data.ZCD_NV_PANID.value, Array.from(nvItems_1.default.panID(options.panID).value))) {
            throw new Error(`Cannot restore backup, panID of backup is different`);
        }
        if (!es6_1.default(backup.data.ZCD_NV_EXTENDED_PAN_ID.value, options.extendedPanID)) {
        ...
        yield znp.request(constants_1.Subsystem.SYS, 'osalNvItemInit', ZCD_NV_NIB, null, [ZnpCommandStatus.SUCCESS, ZnpCommandStatus.NV_ITEM_INITIALIZED]);
        yield znp.request(constants_1.Subsystem.SYS, 'osalNvWrite', backup.data.ZCD_NV_NIB);
        ...
        yield znp.request(constants_1.Subsystem.SYS, 'osalNvItemInit', nvItems_1.default.znpHasConfiguredInit(product), null, [ZnpCommandStatus.SUCCESS, ZnpCommandStatus.NV_ITEM_INITIALIZED]);
        yield znp.request(constants_1.Subsystem.SYS, 'osalNvWrite', nvItems_1.default.znpHasConfigured(product));
        yield znp.request(constants_1.Subsystem.SYS, 'osalNvItemInit', bdbNodeIsOnANetwork, null, [ZnpCommandStatus.SUCCESS, ZnpCommandStatus.NV_ITEM_INITIALIZED]);
